...
import { employeeMarkAvailabilityColumns } from "../../../utils/tablecolumns";
import { theme } from "../../../theme/AppTheme";
import { EmployeeMarkAvailabilityModel } from "../../../models/Model";
...
const MarkAvailability = () => {
    ...
    const [employeeMarkAvailabilityList, setEmployeeMarkAvailabilityList] = useState<Array<EmployeeMarkAvailabilityModel>>([]);
    const [employeeMarkAvailabilityListTemp, setEmployeeMarkAvailabilityListTemp] = React.useState<Array<any>>([]);
            ...
            .then((response: any) => {
                if (response.data && response.data.code === 200) {
                    if (response.data.data) {
                        ...
                        setEmployeeMarkAvailabilityList(arrList);
                        setEmployeeMarkAvailabilityListTemp(arrList);
                        // if (type !== "") {
            ...
            <Container maxWidth="lg">
                <Grid container spacing={{ xs: 1, md: 2 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                    ...
                    <Grid item xs={4} sm={8} md={12}>
                        <Typography variant="h4">EMPLOYEE AVAILABILITY ENTRY</Typography>
                    </Grid>
                ...
                <Grid container spacing={{ xs: 1, md: 2 }} columns={{ xs: 4, sm: 10, md: 15 }}>
                    <Grid item sm={4}>
                        <label><label style={{ color: "#ff0000" }}>*</label>Date of Availability</label>
                    </Grid>
                    <Grid item sm={4}>
                ...
                <Grid item xs={4} sm={8} md={12}>
                    {loading ? (
                        ...
                        <div style={{ height: 500, width: "100%", marginBottom: "20px" }}>
                            {employeeMarkAvailabilityList.length === 0 ? (
                                // <Grid>
                                ...
                                <>
                                    ...
                                    <DataGrid
                                        ...
                                        autoHeight={true}
                                        rows={employeeMarkAvailabilityListTemp}
                                        columns={employeeMarkAvailabilityColumns}
                                        pageSize={pageSize}
                                        ...
                                        onCellClick={(param, e: React.MouseEvent<HTMLElement>) => {
                                            ...
                                            if (param.field === 'action') {
                                                ...
                                                const arrActivity = [...employeeMarkAvailabilityList];
                                                let a: EmployeeMarkAvailabilityModel | undefined = arrActivity.find((el) => el.id === param.row.id);
                                            }
                                        }}
                                        sx={{
