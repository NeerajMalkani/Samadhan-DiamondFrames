...
import { massOfZincCoatingColumns } from "../../../utils/tablecolumns";
import { communication } from "../../../utils/communication";
import { theme } from "../../../theme/AppTheme";
import { GSMModel,DescriptionModel,WidthOfGPCoilModel } from "../../../models/Model";
...
const MassOfZincCoating = () => {
  ...
  const [gsmList, setGsmList] = useState<Array<GSMModel>>([]);
  const [gsmError, setGsmError] = useState("");
  const [isGsmError, setIsGsmError] = useState(false);
  ...
  const [gsmListTemp, setGsmListTemp] = React.useState<Array<any>>([]);
      ...
      <Container maxWidth="lg">
        <Grid container spacing={{ xs: 1, md: 2 }} columns={{ xs: 4, sm: 8, md: 12 }}>
          <Grid item xs={4} sm={8} md={12}>
            <Typography variant="h4">Mass Of Zinc Coating</Typography>
          </Grid>
          <Grid item xs={4} sm={8} md={12} sx={{ borderBottom: 1, paddingBottom: "8px", borderColor: "rgba(0,0,0,0.12)" }}>
            <Typography variant="h6">Mass Of Zinc Coating (Add/Edit) </Typography>
          </Grid>
          <Grid item xs={3} sm={4} md={4} sx={{ mt: 1 }}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>
              <b><label style={{ color: "#ff0000" }}>*</label>GSM</b>
            </Typography>
            <TextField
              fullWidth
              placeholder="GSM"
              ...
              size="small"
              onChange={(e) => {
                setGsm((e.target as HTMLInputElement).value);
                setIsGsmError(false);
                setGsmError("");
              }}
              error={isGsmError}
                ...
                {gsmList.length === 0 ? (
                  ...
                  <>
                    ...
                    <DataGrid
                      ...
                      autoHeight={true}
                      rows={gsmListTemp}
                      columns={massOfZincCoatingColumns}
                      pageSize={pageSize}
                      rowsPerPageOptions={[5, 10, 20]}
                      onPageSizeChange={(newPageSize) => setPageSize(newPageSize)}
                      disableSelectionOnClick
                      onCellClick={(param, e: React.MouseEvent<HTMLElement>) => {
                        const arrActivity = [...gsmList];
                        let a: GSMModel | undefined = arrActivity.find((el) => el.id == param.row.id);
