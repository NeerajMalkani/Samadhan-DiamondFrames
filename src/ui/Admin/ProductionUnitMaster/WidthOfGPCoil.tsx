...
import { widthOfGPCoilColumns } from "../../../utils/tablecolumns";
import { communication } from "../../../utils/communication";
import { theme } from "../../../theme/AppTheme";
import { WidthOfGPCoileModel,DescriptionModel,WidthOfGPCoilModel } from "../../../models/Model";
...
const WidthOfGPCoil = () => {
  ...
  const [widthOfGPCoilList, setWidthOfGPCoilList] = useState<Array<WidthOfGPCoileModel>>([]);
  const [widthOfGPCoilError, setWidthOfGPCoilError] = useState("");
  const [isWidthOfGPCoilError, setIsWidthOfGPCoilError] = useState(false);

  const [description, setDescription] = React.useState("");
  const [descriptionList, setDescriptionList] = useState<Array<DescriptionModel>>([]);
  ...
  const [widthOfGPCoilListTemp, setWidthOfGPCoilListTemp] = React.useState<Array<any>>([]);
      ...
      <Container maxWidth="lg">
        <Grid container spacing={{ xs: 1, md: 2 }} columns={{ xs: 4, sm: 8, md: 12 }}>
          <Grid item xs={4} sm={8} md={12}>
            <Typography variant="h4">Width Of GP Coil</Typography>
          </Grid>
          <Grid item xs={4} sm={8} md={12} sx={{ borderBottom: 1, paddingBottom: "8px", borderColor: "rgba(0,0,0,0.12)" }}>
            <Typography variant="h6">Width Of GP Coil (Add/Edit) </Typography>
          </Grid>
          <Grid item xs={3} sm={4} md={4} sx={{ mt: 1 }}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>
              <b><label style={{ color: "#ff0000" }}>*</label>Width Of GP Coil (Mtr)</b>
            </Typography>
            <TextField
              fullWidth
              placeholder="Width Of GP Coil"
              variant="outlined"
              size="small"
              onChange={(e) => {
                setWidthOfGPCoil((e.target as HTMLInputElement).value);
                setIsWidthOfGPCoilError(false);
                setWidthOfGPCoilError("");
              }}
              error={isWidthOfGPCoilError}
              helperText={widthOfGPCoilError}
              value={widthOfGPCoil}
            />
          </Grid>
          <Grid item xs={3} sm={4} md={4} sx={{ mt: 1 }}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>
              <b><label style={{ color: "#ff0000" }}>*</label>Description</b>
            </Typography>
            <TextField
              fullWidth
          ...
          <Grid item xs={4} sm={8} md={12} sx={{ borderBottom: 1, paddingBottom: "8px", borderColor: "rgba(0,0,0,0.12)" }}>
            <Typography variant="h6">Width Of GP Coil List</Typography>
          </Grid>
          <Grid item xs={4} sm={8} md={12}>
            {loading ? (
                ...
                {widthOfGPCoilList.length === 0 ? (
                  ...
                  <>
                    ...
                    <DataGrid
                      ...
                      autoHeight={true}
                      rows={widthOfGPCoilListTemp}
                      columns={widthOfGPCoilColumns}
                      pageSize={pageSize}
                      rowsPerPageOptions={[5, 10, 20]}
                      onPageSizeChange={(newPageSize) => setPageSize(newPageSize)}
                      disableSelectionOnClick
                      onCellClick={(param, e: React.MouseEvent<HTMLElement>) => {
                        const arrActivity = [...widthOfGPCoilList];
                        let a: WidthOfGPCoilModel | undefined = arrActivity.find((el) => el.id == param.row.id);
